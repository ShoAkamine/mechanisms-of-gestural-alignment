...
from read_labeled_and_pos_target_data import assert_match_betwee_shared_exp_and_actual_utterances
from compute_shared_exp_time_windows import get_from_to_ts
...
def link_all_pairs_shared_expressions_per_num_fribbles(turns_info, exp_info, function_words, pos_func_words):
    ...
    for target_pair in all_pairs:
        for target_fribble in fribbles:
            target_pair_fribble_exp_info = exp_info[(exp_info['pair']==target_pair) & np.array([int(target_fribble) in fribbles for fribbles in exp_info['target_fribbles'].to_list()])]
...
#    fribble_specific_exp_info = linked_exp_info[linked_exp_info['#fribbles'] == 1] replaced by the following line for the zoom project
    fribble_specific_exp_info = linked_exp_info
