...
def extract_all_shared_exp_info(shared_constructions_info, turns_info):
    ...
    for pair in tqdm(shared_constructions_info):
        shared_expressions = shared_constructions_info[pair]['Surface Form'].to_list()
        # shared_expressions = list(map(str.lower, shared_expressions)) # <-- added this line to remove case sensitivity
        ...
        for idx, turns in enumerate(all_turns):
            ...
            from_ts = [turns_info[pair][turn].from_ts for turn in turns]
            to_ts = [turns_info[pair][turn].to_ts for turn in turns]
            establishment_ts = turns_info[pair][establishment_turn].from_ts
        

            this_exp_info = create_exp_info_row(shared_expressions[idx], lengths[idx], targets, target_fribbles, 
                                                num_targets, exp_entropy, speakers, freqs[idx], free_freq[idx], 
                                                priming[idx], spanning_rounds, spanning_time, duration_to_emerge, 
                                                turns_to_emerge, rounds_to_emerge, turns, rounds, establishment_round, 
                                                establishment_turn, first_round, last_round, initiators[idx], 
                                                shared_exp_pos_seq[idx], shared_exp_pos[idx], pair, dataset, 
                                                from_ts, to_ts, establishment_ts)
            if first_row:
...
if __name__ == "__main__":
    dialign_output = 'code/dialign/output_targets_zoom_lemma_lg/'
    turn_info_path = 'code/dialign/targets_dialogues_lemma_lg/'
