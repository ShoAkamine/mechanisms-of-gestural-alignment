...
def build_dialogue_dict(eaf, verbose=False):
    ...
    for tier in eaf:
        ...
        for ann in tier:
            ...
            if tier.ID in ['A_speech', 'B_speech']:
                tier_name = 'speakers'
            elif tier.ID in ['A_po', 'B_po']:
                tier_name = 'speakers'
            else:
                tier_name = tier.ID
...
def normalize_utterance(utterance, dataset_name, dataset, verbose=False):
    ...
    if dataset[dataset_name]['repairs_marker'] in utterance:
        if verbose:
            print('Before: ', utterance)
        for word in utterance.split():
            if dataset[dataset_name]['repairs_marker'] in word and len(word) < 3:
    ...
    if '/' in utterance:
        ...
        if verbose:
            print('After: ', utterance)
    if 'de de de ' in utterance:
        utterance = utterance.replace('de de de ', 'de ')
    if ',' in utterance:    
        utterance = utterance.replace(',', '')
    if 'Ã©' in utterance:    
        utterance = utterance.replace('Ã©', 'é')
    if '<name>' in utterance:
        utterance = utterance.replace('<name>', '')
    if '<naam>' in utterance:   
        utterance = utterance.replace('<naam>', '')
    if '[]' in utterance:
        utterance = utterance.replace('[]', '')
    if '??' in utterance:
        utterance = utterance.replace('??', '')
    if " '" in utterance: # when the utterance contains a word starting with '
        utterance = utterance.replace(" '", " ")
    if " `" in utterance: # when the utterance contains a word starting with `
        utterance = utterance.replace(" `", " ")
    ### fix typos
    if 'paddelstoel' in utterance:
        utterance = utterance.replace('paddelstoel', 'paddenstoel')
    if 'ijshorn' in utterance:
        utterance = utterance.replace('ijshorn', 'ijshoorn')
    if 'ijshoornje' in utterance:
        utterance = utterance.replace('ijshoornje', 'ijshoorntje')
    # if 'op de kop' in utterance: # remove this expression to increase the accuracy of shared expression detection
    #     utterance = utterance.replace('op de kop', '')
...
def clean_and_pos_tag_a_turn(pair_name, turn, dataset_name, dataset, nlp, verbose=False):
    ...
    for idx, token in enumerate(pos_utterance):
        ...
        final_utterance.append(Utterance(token.text, utterance[utterance_idx].from_ts, 
                                         utterance[utterance_idx].to_ts, token.lemma_, 
                                         token.pos_, 
...
def get_interactional_words():
    interaction_words = []
    interaction_words_path = 'code/data/words_lists/interaction_words.txt'
...
def prepare_and_write_target_trials_per_pair(dataset_name, dataset, nlp, alignment_model, alignment_metadata, device, verbose=False, align_speech=False, save_files=True):
    non_used_pairs = []
    speech_and_objects = []
...
def parse_arguments():
    ...
    parser.add_argument('--audio_path_small', type=str, default='P:/workspaces/mld-akamine/working_data/02_marlou_small_cabb_data/chapter2/3_data/referential_task/audio_files/{}_synced_pp{}.wav', help='Audio path template')
    parser.add_argument('--audio_path_zoom', type=str, default='P:/workspaces/mld-akamine/working_data/01_fribble_zoom_experiment/05_data/01_interaction/02_media/audio_files/{}_{}_denoised.wav', help='Audio path template')
    parser.add_argument('--elan_path_small', type=str, default='code/data/cabb_small/elan/*.eaf', help='ELAN files path')
    parser.add_argument('--elan_path_zoom', type=str, default='code/data/zoom/elan/*.eaf', help='ELAN files path')
    ...
    parser.add_argument('--output_path_small', type=str, default='code/dialign/targets_small_lemma_{}/', help='Output path template')
    parser.add_argument('--output_path_zoom', type=str, default='code/dialign/targets_zoom_lemma_{}/', help='Output path template')
...
if __name__ == '__main__':
    ...
    inaudiable_speech_markers = ['(?)', '()', '( )', '(', ')', '(?])', '?]', '[?', ']?', '...'] # the three dots are used to indicate pause
    nlp = spacy.load(spacy_model)
    dataset = {}

    for dataset_name, audio_path, elan_path, output_path, max_trials, pair_name_pattern, non_verbal_sounds_matcher, non_verbal_sounds_marker, targets_marker, target_matcher, repairs_marker in [
        ('small', args.audio_path_small, args.elan_path_small, args.output_path_small, 96, r"\\([\w\d]+)\.", r"\#.*?\#", '#', '*', r"\*.*?\*", '-')
        # ('zoom', args.audio_path_zoom, args.elan_path_zoom, args.output_path_zoom, 96, r"\\([\w\d]+)\.", r"\#.*?\#", '#', '*', r"\*.*?\*", '-')
        # ('large', args.elan_path_large, 192, r"/(\d+)_", r"\*.*?\*", '*', '#', r"\#.*?\#", ']')
    ]:
        dataset[dataset_name] = {
            'audio_path': audio_path,
            'dataset_path': elan_path,
            'output_path': output_path.format(spacy_model.split('_')[-1]),
            'name': dataset_name,
        ...
        ### for the zoom dataset, we will remove the answers to avoid confusion between the object numbers/letters and references to objects (e.g., "with an arm looking like the letter L")
        ...
        dataset[dataset_name]['dutch_numbers'] = {f'{targets_marker}{i}{targets_marker}': name for i, name in enumerate(['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''], 1)}
        dataset[dataset_name]['dutch_capital_letters'] = {f'{targets_marker}{chr(65+i)}{targets_marker}': '' for i in range(16)}
